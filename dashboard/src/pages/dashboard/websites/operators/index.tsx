...
export default function WebsiteOperatorsPage() {
  ...
  const { operators, meta, isLoading } = useOperators(currentPage, websiteId)
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">اوپراتورها</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            این قسمت مربوط به مدیریت اوپراتورهای وبسایت شما می باشد. ازین قسمت
            می توانید اوپراتورهای جدید به داشبورد دعوت کنید و وضعیت اوپراتورها
            را مدیریت کنید.
          </p>
        </div>
        ...
        <Button
          nativeButton={false}
          render={
            <Link to={`/dashboard/websites/${websiteId}/operators/create`}>
              ایجاد اوپراتور جدید
            </Link>
          }
        />
            ...
            >
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge
                      ...
                      className={cn(
                        ...
                        "text-xs",
                        "font-normal",
                        `${operator.status === "active" ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : ""}`,
                        `${operator.status === "inactive" ? "border-yellow-500/20 bg-yellow-500/10 text-yellow-600 dark:text-yellow-400" : ""}`,
                        `${operator.status === "suspended" ? "border-red-500/20 bg-red-500/10 text-red-600 dark:text-red-400" : ""}`
                      )}
                    >
                      <ShieldCheck />
                      {operator.status === "active" && <span>فعال</span>}
                      {operator.status === "inactive" && <span>غیرفعال</span>}
              ...
              >
                <Button
                  ...
                  render={
                    <Link
                      to={`/dashboard/websites/${websiteId}/operators/${operator.id}/edit`}
                    >
                ...
                <Button
                  ...
                  render={
                    <Link
                      to={`/dashboard/websites/${websiteId}/operators/${operator.id}/show`}
                    >
      ...
      {!isLoading && operators?.length === 0 && (
        <Card className="flex flex-col items-center justify-center p-8 text-center">
          ...
          <h3 className="mt-4 text-base font-semibold">
            هیچ اوپراتوری یافت نشد
          </h3>
          <p className="mt-1 text-sm text-muted-foreground">
            هنوز اوپراتوری اضافه نکرده‌اید. با کلیک بر روی دکمه زیر اولین
